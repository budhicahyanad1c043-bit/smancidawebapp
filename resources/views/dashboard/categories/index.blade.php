                    ...
                    <tr class="hover:bg-slate-50/80 transition-colors">
                        ...
                        <td class="px-6 py-4 text-center">
                            <div class="flex items-center justify-center space-x-2">
                                ...
                                <form action="{{ route('categories.destroy', $category->id) }}" method="POST" class="delete-form-category">
                                    @csrf @method('DELETE')
                                    <button type="button" class="p-1.5 text-rose-600 hover:bg-rose-50 border border-rose-100 rounded-md transition btn-delete-category" title="Hapus">
...
<script>
    ...
    document.querySelectorAll('.btn-delete-category').forEach(button => {
        button.addEventListener('click', function() {
            const form = this.closest('.delete-form-category');
            
            Swal.fire({
                title: 'Apakah Anda yakin?',
                text: "Data kategori ini akan dihapus permanen dari sistem! Postingan tetap berjalan",
                icon: 'warning',
